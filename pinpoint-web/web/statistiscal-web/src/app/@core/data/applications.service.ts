...
export class ApplicationsService {
  ...
  getApplications(): Observable<string[]> {
    const requestUrl: string = '/rest/applications';  // 后端url
  ...
  getApplicationsBySimilar(term: string): Observable<string[]> {
    const requestUrl = '/rest/applications/name';   // 后端url

    if (!term.trim()) {
      return of([]);
    }
    //
    const applicationMockUrl: string = this.mockUrl;
    // 相似模块查询
    return this.http.get<string[]>(requestUrl + '/' + term)  // requestUrl + "/" + term
      .pipe(
